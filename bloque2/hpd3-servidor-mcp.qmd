...
En HPD 2 usaste `create_tool_calling_agent` de LangChain para crear el agente.
...
::: {.callout-warning}
## ⏱️ Esta es la celda más compleja del taller
...
Marca con ✅ lo que has implementado y ✏️ lo que añadirías en producción:

| Riesgo | Implementado | Mitigación |
|---|---|---|
| SQL Injection vía prompt | ✏️ | Solo permitir SELECT, sanitizar query |
| Data leakage (demasiadas filas) | ✏️ | Limitar a 30 filas, 2000 caracteres |
| Path traversal (si expusieras archivos) | ✏️ | Validar rutas con `os.path.abspath` |
| Denegación de servicio | ✏️ | Timeout en queries (máx. 5s) |
| Acceso no autorizado | ✏️ | API keys en transporte, logs de auditoría |
| Escalada de privilegios | ✏️ | Cada tool accede solo a lo que necesita |
:::
...
graph TD
    ...
    J[🛡️ Sanitización<br/>+ límite filas<br/>+ logs] -.-> D
...
Investiga cómo lanzar el servidor MCP como script independiente (`python servidor_mcp.py`) y conectarte desde un cliente usando `fastmcp.Client`. El código base está en [02d-mcp.qmd](02d-mcp.qmd#servidor-mcp-completo-para-ejecutar-como-script). Adapta tu servidor a ese formato y pruébalo con Ollama local.
