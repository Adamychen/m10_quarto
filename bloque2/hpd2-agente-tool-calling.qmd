...
[![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/Adamychen/m10_quarto/blob/main/notebooks/bloque2/hpd2-agente-tool-calling.ipynb)
...
# Parte 2 — Dataset: Titanic {#sec-hpd2-p2}
...
# Parte 3 — Herramientas del agente {#sec-hpd2-p3}

## Tool 1: consulta_pandas {#sec-hpd2-tool-pandas}
...
## Tool 2: describe_schema {#sec-hpd2-tool-schema}

Fuente de verdad del esquema: el agente **no ve el DataFrame**, solo sabe lo que le dicen los
docstrings. Si duda del nombre exacto de una columna (o no le queda claro qué contiene),
debe llamar a `describe_schema` antes de consultar. Esta tool ya viene implementada:
...
## Tools de visualización {#sec-hpd2-tools-viz}
...
### Tool libre: código matplotlib arbitrario {#sec-hpd2-tool-libre}
...
# Parte 4 — Construir el agente ReAct {#sec-hpd2-p4}

## Prompt del agente {#sec-hpd2-prompt-agente}
...
## Crear el agente {#sec-hpd2-crear-agente}
...
# Parte 5 — Batería de pruebas {#sec-hpd2-p5}
...
## Tabla de evaluación {#sec-hpd2-evaluacion}
...
# Ejercicios {#sec-hpd2-ejercicios}

::: {.callout-tip icon=false}

### Para afianzar lo aprendido (~45 min) {#sec-hpd2-ejercicios-afianzar}
...
### Para seguir explorando (opcional) {#sec-hpd2-ejercicios-explorando}
...
`create_agent` no guarda estado entre llamadas: cada `invoke` parte de cero. Para dar memoria
al agente, acumula el historial en el campo `messages` de la siguiente invocación (reutiliza el
`result["messages"]` de la llamada anterior y añade tu nueva pregunta). Haz 3 preguntas
encadenadas donde la segunda y tercera dependan del contexto de las anteriores. Ejemplo:
*"¿Cuántos pasajeros viajaban en primera clase?"* → *"¿Y cuántos de ellos sobrevivieron?"* →
