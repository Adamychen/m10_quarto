...
LangChain abstrae el bucle ReAct con `create_tool_calling_agent`:
...
if not LLM_KEY:
    ...
    prompt = ChatPromptTemplate.from_messages([
        ("system", "Eres un asistente corporativo con acceso a herramientas. Responde de forma concisa."),
        ("placeholder", "{chat_history}"),
        ("human", "{input}"),
        ("placeholder", "{agent_scratchpad}")
    ])
    agente = create_tool_calling_agent(llm, tools, prompt)
    executor = AgentExecutor(agent=agente, tools=tools, verbose=False, max_iterations=5, handle_parsing_errors=True)
...
if LLM_KEY:
    prompt = ChatPromptTemplate.from_messages([
        ("system", "Eres un analista de datos. Usa la herramienta consulta_pandas para responder preguntas sobre el DataFrame de ventas. Responde en español."),
        ("placeholder", "{chat_history}"),
        ("human", "{input}"),
        ("placeholder", "{agent_scratchpad}")
    ])
    agente = create_tool_calling_agent(llm, [consulta_pandas], prompt)
    executor = AgentExecutor(agent=agente, tools=[consulta_pandas], verbose=False, max_iterations=5, handle_parsing_errors=True)
