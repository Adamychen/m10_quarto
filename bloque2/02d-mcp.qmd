...
Imagina que tienes 5 fuentes de datos distintas (una base de datos SQLite, una API REST de CRM, archivos CSV locales, Google Drive, un servicio meteorológico). Conectar todas a un LLM implica escribir un adaptador personalizado para cada una. Cada framework (LangChain, LlamaIndex, OpenAI...) tiene su propia forma de definir herramientas. Resultado: **un lío de integraciones ad hoc que no escalan**.

**MCP** es un protocolo abierto (liderado por Anthropic) que estandariza cómo los LLMs se conectan a herramientas y fuentes de datos externas. Funciona como un "USB-C para LLMs": una interfaz única para todo.

```{mermaid}
graph TD
    subgraph "Sin MCP — M×N integraciones"
        L1[Claude] --> T1[(BD)]
        L1 --> T2[API REST]
        L2[Qwen] --> T1
        L2 --> T2
    ...
    subgraph "Con MCP — N+M integraciones"
        LC[Claude] --> MC[MCP Server BD]
        LC --> MR[MCP Server API]
        LQ[Qwen] --> MC
        LQ --> MR
        LG[GPT] --> MC
        LG --> MR
    ...
    style MR fill:#e65100,color:#fff
```
...
graph TD
    ...
    H[🛡️ Sanitización<br/>+ límites + logs] -.-> D
