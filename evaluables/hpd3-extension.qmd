...
Crea un agente con LangChain (`create_tool_calling_agent`) donde las tools no sean funciones Python decoradas con `@tool`, sino wrappers que invoquen las tools de tu servidor MCP. El agente debe responder preguntas sobre la base de datos corporativa usando exclusivamente las tools MCP.
...
if not LLM_KEY:
    ...
    prompt = ChatPromptTemplate.from_messages([
        ("system", "Eres un asistente corporativo. Usa las tools para responder. Responde en español."),
    ...
    eval3_integracion = result['output']
```
